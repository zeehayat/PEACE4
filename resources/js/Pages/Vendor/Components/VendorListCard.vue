    ...
    <div class="bg-surface rounded-xl shadow-lg border border-ink-200 p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:scale-[1.01] relative">
        ...
        <div class="mt-4 pt-4 border-t border-ink-200">
            ...
            <div class="flex items-center justify-between">
                ...
                <p class="font-semibold text-sm text-ink-900">{{ vendor.name ?? 'N/A' }}</p>
            </div>
            <div class="flex items-center justify-between">
                <p class="text-sm text-ink-500">Contact Person</p>
                <p class="font-semibold text-sm text-ink-900">{{ vendor.contact_person ?? 'N/A' }}</p>
            </div>
            <div class="flex items-center justify-between">
                <p class="text-sm text-ink-500">Phone</p>
                <p class="font-semibold text-sm text-ink-900">{{ vendor.phone ?? 'N/A' }}</p>
            </div>
            <div class="flex items-center justify-between">
                <p class="text-sm text-ink-500">Email</p>
                <p class="font-semibold text-sm text-ink-900">{{ vendor.email ?? 'N/A' }}</p>
            </div>
        </div>
    </div>
</template>
