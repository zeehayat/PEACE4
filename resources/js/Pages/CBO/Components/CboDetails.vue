...
<template>
    <div class="space-y-6">
        <!-- Header / Identity -->
        <section class="bg-gradient-to-r from-accent-50 to-blue-50 border border-accent-100 rounded-2xl p-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-2xl md:text-3xl font-extrabold text-accent-900 tracking-tight">
                        {{ cbo?.name || cbo?.title || 'CBO' }}
                    </h2>
                    <p class="text-sm text-ink-600 mt-1">
                        <span v-if="cbo?.reference_code" class="mr-3"><span class="font-medium text-ink-700">Ref:</span> {{ cbo.reference_code }}</span>
                        <span v-if="cbo?.district" class="mr-3"><span class="font-medium text-ink-700">District:</span> {{ cbo.district }}</span>
                        <span v-if="cbo?.gender" class="mr-3"><span class="font-medium text-ink-700">Gender:</span> {{ cbo.gender }}</span>
                        <span v-if="cbo?.date_of_formation"><span class="font-medium text-ink-700">Formed:</span> {{ formatDate(cbo.date_of_formation) }}</span>
                    </p>
                </div>

                <div class="flex items-center gap-2">
                    <button class="px-4 py-2 rounded-xl text-sm font-semibold bg-surface border border-accent-200 shadow-sm hover:bg-accent-50">
                        Print
                    </button>
                    <button class="px-4 py-2 rounded-xl text-sm font-semibold bg-accent-600 text-white shadow hover:bg-accent-700">
                        Export CSV
                    </button>
                </div>
            </div>
        ...
        <section>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div
                    v-for="(s, i) in stats"
                    :key="i"
                    class="rounded-2xl border border-ink-200 bg-surface p-4 shadow-sm"
                >
                    <div class="text-sm text-ink-500">{{ s.label }}</div>
                    <div class="mt-1 text-2xl font-bold text-ink-900">{{ s.value }}</div>
                </div>
            </div>
        </section>

        <!-- Dialogues -->
        <section class="bg-surface rounded-2xl border border-ink-200 shadow-sm">
            <header class="p-5 border-b border-ink-100 flex items-center justify-between">
                <h3 class="text-xl font-semibold text-ink-800 flex items-center gap-2">
                ...
                <div class="text-sm text-ink-500">Total participants: {{ sumBy(dialogues, 'participants') }}</div>
            </header>

            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead>
                    <tr class="bg-paper-50 text-ink-600">
                    ...
                    <tr v-if="!dialogues.length">
                        <td colspan="4" class="px-4 py-6 text-center text-ink-500">No dialogues recorded.</td>
                    </tr>
                    <tr v-for="d in dialogues" :key="d.id" class="border-t border-ink-100">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <a v-for="m in d?.attachments || d?.media || []" :key="m.id || m.url" :href="m.url || m.original_url" target="_blank" class="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-ink-200 hover:bg-paper-50">
        ...
        <section class="bg-surface rounded-2xl border border-ink-200 shadow-sm">
            <header class="p-5 border-b border-ink-100 flex items-center justify-between">
                ...
                <div class="text-sm text-ink-500">Total participants: {{ sumBy(trainings, 'participants') }}</div>
            </header>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr v-if="!trainings.length">
                        <td colspan="5" class="px-4 py-6 text-center text-ink-500">No trainings recorded.</td>
                    </tr>
                    <tr v-for="t in trainings" :key="t.id" class="border-t border-ink-100">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <a v-for="m in t?.attachments || t?.media || []" :key="m.id || m.url" :href="m.url || m.original_url" target="_blank" class="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-ink-200 hover:bg-paper-50">
                                    <span class="i-file w-4 h-4" />
                                    <span class="truncate max-w-[10rem]">{{ m.name || m.file_name || 'file' }}</span>
                                </a>
                            </div>
        ...
        <section class="bg-surface rounded-2xl border border-ink-200 shadow-sm">
            <header class="p-5 border-b border-ink-100 flex items-center justify-between">
                ...
                <div class="text-sm text-ink-500">Total participants: {{ sumBy(exposureVisits, 'participants') }}</div>
            </header>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr v-if="!exposureVisits.length">
                        <td colspan="5" class="px-4 py-6 text-center text-ink-500">No exposure visits recorded.</td>
                    </tr>
                    <tr v-for="e in exposureVisits" :key="e.id" class="border-t border-ink-100">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-2">
                                <a v-for="m in e?.attachments || e?.media || []" :key="m.id || m.url" :href="m.url || m.original_url" target="_blank" class="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-ink-200 hover:bg-paper-50">
                                    <span class="i-file w-4 h-4" />
                                    <span class="truncate max-w-[10rem]">{{ m.name || m.file_name || 'file' }}</span>
                                </a>
                            </div>
        ...
        <!-- Attachments (Gallery) -->
        <section class="bg-surface rounded-2xl border border-ink-200 shadow-sm">
            <header class="p-5 border-b border-ink-100">
                <h3 class="text-xl font-semibold text-ink-800 flex items-center gap-2">
            ...
            <div v-if="!attachments.length" class="p-6 text-center text-ink-500">
            ...
            <div v-else class="p-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                <a
                    ...
                    target="_blank"
                    class="group block rounded-xl border border-ink-200 overflow-hidden bg-paper-50 hover:bg-paper-100"
                >
                    <div class="aspect-video bg-surface flex items-center justify-center overflow-hidden">
                        <img
                        ...
                        <div v-else class="p-4 text-center">
                            ...
                            <div class="mt-2 text-xs text-ink-600 truncate">
                                {{ m.name || m.file_name || 'file' }}
                            </div>
                        </div>
                    </div>
                    <div class="p-3 border-t border-ink-200">
                        <div class="text-sm font-medium text-ink-800 truncate">{{ m.name || m.file_name || 'file' }}</div>
                        <div class="text-xs text-ink-500 truncate">{{ m.mime_type || fileExt(m?.name || m?.file_name) }}</div>
                    </div>
                </a>
            </div>
        </section>
