...
const props = defineProps({
    ...
    progress: { type: Object, default: null }, // Existing progress object for edit mode
...
const initializeTransformers = (transformersJson) => {
    // Transformers are stored as JSON: [{"kva": 500, "count": 2}, {"kva": 1000, "count": 1}]
    // We need to convert this into a reactive structure for the form,
    // e.g., an array of objects for easier v-for.
...
const form = useForm({
    ...
    progress_type: props.progress?.progress_type || 'Civil', // Initialize to Civil by default
    ...
    // T&D remarks is optional, can reuse general remarks or have separate field if needed

    attachments: [],
    removed_attachments: [],
    // existing_attachments is managed by a separate ref
});

const existingAttachments = ref(props.progress?.attachments || []);


// Watch for prop changes (e.g., when modal is reused for a different progress record)
watch(
    () => props.progress,
    (newProgress) => {
        if (props.mode === 'edit' && newProgress) {
            ...
            // Re-initialize T&D fields from newProgress.t_and_d_work
            ...
            form.projectable_id = props.projectableId;
            form.projectable_type = props.projectableType;
            form.progress_type = 'Civil'; // Default to Civil for new
            form.attachments = [];
            form.removed_attachments = [];
...
// Functions to add/remove transformer entries in the form
...
const submitForm = () => {
    ...
    for (const key in form.data()) {
        ...
        if (['step_up_transformers', 'step_down_transformers'].includes(key)) {
            // Filter out empty/invalid entries from the array before JSON stringify
