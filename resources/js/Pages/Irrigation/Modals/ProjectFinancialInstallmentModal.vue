        ...
        <div class="p-6 overflow-y-auto max-h-[85vh]">
            <div v-if="!showForm" class="space-y-4">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-semibold text-ink-800">Existing Financial Installments</h3>
                    <PrimaryButton @click="openCreateForm">
                        + Add New Installment
                    </PrimaryButton>
                </div>

                <div v-if="isLoadingInstallments" class="text-center py-8 text-ink-500">
                    Loading financial installments...
                </div>

                <div v-else-if="financialInstallments.length === 0" class="text-center py-8 text-ink-500">
                    No financial installments recorded yet.
                </div>

                <div v-else class="grid grid-cols-1 gap-4">
                    <div v-for="installment in financialInstallments" :key="installment.id" class="bg-surface border border-ink-200 rounded-lg p-4 shadow-sm">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                <p class="text-lg font-semibold text-accent-700">Installment #{{ installment.installment_number }} ({{ installment.payment_for ?? 'N/A' }})</p>
                                <p class="text-sm text-ink-600">Amount: {{ formatCurrency(installment.installment_amount) }} on {{ formatNullableDate(installment.installment_date) }}</p>
                                <p v-if="installment.cheque_no" class="text-xs text-ink-500 mt-0.5 font-medium">Cheque Number: {{ installment.cheque_no }}</p>
                            </div>
                        ...
                        <div v-if="installment.remarks" class="text-sm text-ink-700 mt-2">
                            Remarks: <span class="text-ink-600 prose max-w-none" v-html="installment.remarks"></span>
                        </div>
