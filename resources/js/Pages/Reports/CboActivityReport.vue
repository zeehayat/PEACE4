<template>
    <div class="bg-gradient-to-br from-paper-50 to-paper-200 min-h-screen p-8 font-sans antialiased text-ink-800">
        <header class="text-center mb-10">
            <h1 class="text-5xl font-extrabold text-accent-800 tracking-tight leading-tight">CBO Activity Report</h1>
            <p class="text-xl text-ink-600 mt-2">Comprehensive Overview of Community Based Organizations' Engagements</p>
            <div class="mt-4 text-ink-500 text-sm">
        ...
        <div class="bg-surface shadow-lg rounded-xl p-6 mb-12 border border-ink-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
                ...
                <div class="relative">
                    <label for="from_date" class="absolute -top-2 left-3 text-xs text-ink-500 bg-surface px-1">From Date</label>
                    <input id="from_date" v-model="filters.from" type="date" class="input-modern pt-4" />
                </div>
                <div class="relative">
                    <label for="to_date" class="absolute -top-2 left-3 text-xs text-ink-500 bg-surface px-1">To Date</label>
        ...
        <section v-if="cbos.length" class="space-y-10">
            <div v-for="cbo in cbos" :key="cbo.id" class="bg-surface shadow-xl rounded-xl p-8 border border-ink-200 transform transition-all duration-300 hover:scale-[1.005] hover:shadow-2xl">
                <div class="flex items-center justify-between mb-4 border-b pb-4">
                    <h2 class="text-3xl font-extrabold text-accent-700">CBO: {{ cbo.reference_code }}</h2>
                    <span class="text-sm font-medium text-ink-500 bg-paper-100 px-3 py-1 rounded-full">ID: {{ cbo.id }}</span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-ink-700 mb-6 text-base">
                    <div>
                        <strong class="text-ink-600">District:</strong> {{ cbo.district }}
                    </div>
                    <div>
                        <strong class="text-ink-600">Gender:</strong> {{ cbo.gender.charAt(0).toUpperCase() + cbo.gender.slice(1) }}
                    </div>
                    <div>
                        <strong class="text-ink-600">Formed:</strong> {{ cbo.date_of_formation }}
                    </div>
                </div>
                ...
                <div class="space-y-8 mt-6">
                    ...
                    <div class="bg-blue-50/70 p-5 rounded-lg border border-blue-200">
                        ...
                        <ul v-if="cbo.dialogues?.length" class="space-y-2 text-ink-700">
                        ...
                        <p v-else class="text-ink-500 italic p-3 text-center">No dialogues recorded for this CBO.</p>
                    </div>
                    ...
                    <div class="bg-green-50/70 p-5 rounded-lg border border-green-200">
                        ...
                        <ul v-if="cbo.trainings?.length" class="space-y-2 text-ink-700">
                        ...
                        <p v-else class="text-ink-500 italic p-3 text-center">No trainings recorded for this CBO.</p>
                    </div>
                    ...
                    <div class="bg-purple-50/70 p-5 rounded-lg border border-purple-200">
                        ...
                        <ul v-if="cbo.exposureVisits?.length" class="space-y-2 text-ink-700">
                        ...
                        <p v-else class="text-ink-500 italic p-3 text-center">No exposure visits recorded for this CBO.</p>
                    </div>
                </div>
            </div>
        </section>

        <section v-else class="bg-surface shadow-lg rounded-xl p-8 text-center text-ink-500 text-lg">
...
.btn-primary-filled {
    @apply inline-flex items-center justify-center px-6 py-2.5 border border-transparent text-base font-semibold rounded-lg shadow-md text-white bg-accent-600 hover:bg-accent-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent-500 transition-colors duration-200 ease-in-out transform hover:scale-105;
}

/* Success Button with Outline */
.btn-success-outline {
    @apply inline-flex items-center justify-center px-6 py-2.5 border border-green-600 text-base font-semibold rounded-lg shadow-md text-green-700 bg-surface hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200 ease-in-out transform hover:scale-105;
}
