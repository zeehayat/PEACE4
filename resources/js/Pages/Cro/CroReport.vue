    ...
    <AppLayout title="CRO Report">
        <template #header>
            <div class="flex flex-col gap-1">
                <h2 class="font-semibold text-xl text-slate-800 leading-tight">CRO Report</h2>
                <p class="text-sm text-slate-500">Comprehensive report of Community Representative Organizations.</p>
            </div>
        </template>
        ...
        <div class="py-10">
            <div class="w-full px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <!-- Filters Section -->
                <div class="bg-white shadow-sm sm:rounded-lg p-6 border border-slate-100">
                    <div class="flex flex-wrap items-end gap-6 justify-between">
                        <div class="flex flex-wrap gap-4 items-end flex-grow">
                            ...
                            <div class="flex gap-2 pb-1">
                                ...
                                <button
                                    type="button"
                                    class="h-10 px-4 py-2 bg-slate-100 text-slate-700 rounded-md text-sm hover:bg-slate-200"
                                    @click="clearFilters"
                                >
                                    Clear
                                </button>
                ...
                <!-- Data Table -->
                <div class="bg-white shadow-sm sm:rounded-lg border border-slate-100">
                        ...
                        <table class="min-w-max divide-y divide-slate-200 text-sm">
                            <thead class="bg-slate-50 sticky top-0 z-10 shadow-sm">
                                <tr>
                                    ...
                                    class="px-3 py-3 text-left font-semibold text-slate-600 uppercase tracking-wider cursor-pointer hover:bg-slate-100 whitespace-nowrap"
                                    @click="sortBy(key)">
                            ...
                            <tbody class="bg-white divide-y divide-slate-100">
                                <tr v-if="!sortedRows.length">
                                    <td colspan="100" class="px-4 py-8 text-center text-slate-500">No records found.</td>
                                </tr>
                                <tr v-for="row in sortedRows" :key="row.id" class="hover:bg-slate-50 transition-colors">
                                    ...
                                    ]" :key="key" class="px-3 py-2 whitespace-nowrap text-slate-700 border-r border-slate-50 last:border-r-0">
                                        {{ row[key] }}
                                    </td>
                                </tr>
                            </tbody>
