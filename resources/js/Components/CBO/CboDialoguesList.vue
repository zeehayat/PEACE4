...
<template>
    <div class="p-6">
        ...
        <div v-if="cbo.dialogues && cbo.dialogues.length > 0" class="space-y-6">
            <div v-for="dialogue in cbo.dialogues" :key="dialogue.id" class="bg-paper-50 border border-ink-200 rounded-lg p-4 shadow-sm">
                <div class="flex justify-between items-center mb-2">
                    <p class="text-lg font-semibold text-ink-800">Date: <span class="font-normal">{{ dialogue.date_of_dialogue }}</span></p>
                    <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                        {{ dialogue.participants }} Participants
                    </span>
                </div>
                <p v-if="dialogue.remarks" class="text-ink-700 mb-3">{{ dialogue.remarks }}</p>

                <div v-if="dialogue.media && dialogue.media.length > 0" class="mt-4 border-t border-ink-200 pt-3">
                    ...
                    <ul class="space-y-2 text-sm text-ink-600">
                        <li v-for="mediaItem in dialogue.media" :key="mediaItem.id" class="flex items-center gap-2">
                            ...
                            <a :href="mediaItem.original_url" target="_blank" class="text-accent-600 hover:underline truncate">
                                {{ mediaItem.file_name }} ({{ (mediaItem.size / 1024).toFixed(1) }} KB)
                            </a>
                        </li>
                    </ul>
                </div>
                <p v-else class="text-ink-500 text-sm italic mt-4">No attachments for this dialogue.</p>
            </div>
        </div>
