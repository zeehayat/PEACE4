    ...
    <div class="cbo-card relative">
        <div class="absolute top-4 right-4 z-50 action-menu-container">
            <button @click.stop="handleToggleActionMenu($event)"
                    class="p-2 text-ink-500 hover:text-ink-900 rounded-full hover:bg-paper-200/70 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent-500 transition-opacity">
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                <div v-if="openActionMenuId === cbo.id"
                     :class="['origin-top-right absolute w-56 rounded-xl shadow-xl bg-surface ring-1 ring-black ring-opacity-5 z-30 divide-y divide-ink-100', menuDirection === 'up' ? 'bottom-full mb-2 right-0' : 'top-full mt-2 right-0']">
                    <div class="py-1 text-sm text-ink-700">
                        <button @click="emit('edit', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                        ...
                        <button @click="emit('open-details', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-eye"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>
                            View Details
                        </button>
                        <a :href="mapLinkUrl" target="_blank" rel="noopener noreferrer" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                    ...
                    <div class="py-1 text-sm text-ink-700">
                        <button @click="emit('open-dialogue', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                        ...
                        <button @click="emit('view-dialogues', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                        ...
                        <button @click="emit('open-training', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                        ...
                        <button @click="emit('view-trainings', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                        ...
                        <button @click="emit('open-exposure', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2">
                        ...
                        <button @click="emit('view-exposure-visits', cbo)" class="w-full text-left block px-4 py-2 hover:bg-paper-100 flex items-center gap-2"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-map-pin"><path d="M12 2v20M18 6H6M22 18H2"/></svg>
        ...
        <div class="mb-4">
            <div class="flex items-center justify-between pb-3 border-b border-ink-200">
                <h3 class="text-xl font-bold text-accent-700">{{ cbo.reference_code }}</h3>
                <span class="text-xs font-medium text-ink-500 bg-paper-100 px-2 py-1 rounded-full">ID: {{ cbo.id }}</span>
            </div>
            <div class="mt-4 rounded-lg overflow-hidden border border-ink-200 shadow-sm">
...
<style scoped>
.cbo-card {
    @apply bg-surface shadow-xl rounded-xl p-6 border border-ink-200
...
/* Tooltip Styling */
.tooltip {
    @apply absolute bottom-full left-1/2 -translate-x-1/2 mb-2 text-xs bg-paper-800 text-white px-2.5 py-1.5 rounded-md
